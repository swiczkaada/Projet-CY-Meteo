...
int main(int argc, char *argv[]){
    ...
    char *fichierEntree = malloc(sizeof(char) * 128);
    int mode_tri = -1;
    if (!fichierEntree) {
        printf("ERREUR ALLOCATION MEMOIRE\n");
        exit(4);
    }
    char *fichierSortie = malloc(sizeof(char) * 128);
    if (!fichierSortie) {
        printf("ERREUR ALLOCATION MEMOIRE\n");
        exit(4);
    }
    char *optionDonnee = malloc(sizeof(char) * 3);
    if (!optionDonnee) {
        printf("ERREUR ALLOCATION MEMOIRE\n");
        exit(4);
    }

    while ((option = getopt(argc, argv, "f:o:rk:-")) != -1) {
        switch (option)
            {
            case 'f':
                optionEntree = 1;
                sprintf(fichierEntree, "%s", optarg);
                break;
            case 'o':
                optionSortie = 1;
 ...
 // Verification de la presence des options obligatoires
    ...
    if (!strcmp(optionDonnee, "h")) {
        // altitude
        lectureFichierAltitude(fichierEntree, fichierSortie, mode_tri);
    }
    else if (!strcmp(optionDonnee, "w")) {
        // vent
        lectureFichierVent(fichierEntree, fichierSortie, mode_tri);
    }
    else if (!strcmp(optionDonnee, "m")) {
        // hum
        lectureFichierHumidite(fichierEntree, fichierSortie, mode_tri);
    }
    else if (!strcmp(optionDonnee, "p1")) {
        // p1
        lectureFichierP1(fichierEntree, fichierSortie, mode_tri);
    }
    else if (!strcmp(optionDonnee, "p2")) {
        // p2
        lectureFichierP2(fichierEntree, fichierSortie, mode_tri);
    }
    else if (!strcmp(optionDonnee, "p3")) {
        // p3
        lectureFichierP3(fichierEntree, fichierSortie, mode_tri);
    }
    else if (!strcmp(optionDonnee, "t1")) {
        // t1
        lectureFichierT1(fichierEntree, fichierSortie, mode_tri);
    }
    else if (!strcmp(optionDonnee, "t2")) {
        // t2
        lectureFichierT2(fichierEntree, fichierSortie, mode_tri);
    }
    else if (!strcmp(optionDonnee, "t3")) {
        // t3
        lectureFichierT3(fichierEntree, fichierSortie, mode_tri);
    }
    else {
...
void lectureFichierAltitude(char *nom_fichier, char *nom_fichier_sortie, int mode) {
    ...
    while (!feof(fichier)) {
        ...
        if (mode == 0) {
            arbre = insereAVLAltitude(arbre, valeur2, valeur1, 0, 0, coordonees, &h);
        }
        else if (mode == 1) {
            arbre = insereABR(arbre, valeur2, valeur1, 0, 0, coordonees, 0);
        }
        else {
...
void lectureFichierHumidite(char *nom_fichier, char *nom_fichier_sortie, int mode) {
    ...
    while (!feof(fichier)) {
        ...
        if (mode == 0){
            arbre = insereAVLHumidite(arbre, valeur2, valeur1, 0, 0, coordonees, &h);
        }
        else if (mode == 1){
            arbre = insereABRHumidite(arbre, valeur2, valeur1, 0, 0, coordonees);
        }
        else{
            //chainon
...
PArbre parcoursHumidite(PArbre a, PArbre b) {
    ...
    // printf("insertion");
    int h = 0;
    b = insereAVL(b, a->elmt, a->f, a->g, a->h, a->i, &h);
    // printf("insere");
...
PArbre parcoursHumiditeABR(PArbre a, PArbre b) {
    ...
    // printf("insertion");
    b = insereABR(b, a->elmt, a->f, a->g, a->h, a->i, a->j);
    // printf("insere");
...
void lectureFichierVent(char *nom_fichier, char *nom_fichier_sortie, int mode) {
    ...
    while (!feof(fichier)) {
        ...
        if (mode == 0){
            arbre = insereAVLVent(arbre, valeur1, valeur2, valeur3, 0, coordonees, 0, &h);
        }
        else if (mode == 1){
            arbre = insereABRVent(arbre, valeur1, valeur2, valeur3, 0, coordonees, 0);
        }
        else{
            //chainon
...
void lectureFichierP1(char *nom_fichier, char *nom_fichier_sortie, int mode) {
    ...
    while (!feof(fichier)) {
        ...
        if (mode == 0){
            arbre = insereAVLP1(arbre, valeur1, valeur2, 1, 0, NULL, 0, &h);
        }
        else if (mode == 1){
            arbre = insereABRP1(arbre, valeur1, valeur2, 1, 0, NULL, 0);
        }
        else{
            //chainon
...
void lectureFichierT1(char *nom_fichier, char *nom_fichier_sortie, int mode) {
    ...
    while (!feof(fichier)) {
        ...
        if (mode == 0){
            arbre = insereAVLP1(arbre, valeur1, valeur2, 1, 0, NULL, 0, &h); //on appelle insereAVLP1 car c'est le même que T1
        }
        else if (mode == 1){
            arbre = insereABRP1(arbre, valeur1, valeur2, 1, 0, NULL, 0);
        }
...
void lectureFichierP2(char *nom_fichier, char *nom_fichier_sortie, int mode) {
    ...
    while (!feof(fichier)) {
        ...
        if (mode == 0){
            arbre = insereAVLP2(arbre, 0, valeur2, 1, 0, coordonees, 0, &h);
        }
        else if (mode == 1){
            arbre = insereABRP2(arbre, 0, valeur2, 1, 0, coordonees, 0);
        }
        else{
            //chainon
...
void lectureFichierT2(char *nom_fichier, char *nom_fichier_sortie, int mode) {
    ...
    while (!feof(fichier)) {
        ...
        if (mode == 0){
            arbre = insereAVLP2(arbre, 0, valeur2, 1, 0, coordonees, 0, &h); //La même pour T2 et P2
        }
        else if (mode == 1){
            arbre = insereABRP2(arbre, 0, valeur2, 1, 0, coordonees, 0); //La même pour T2 et P2
        }
        else{
            //chainon
            liste = insertionListeP2(liste, 0, valeur2, 1, 0, coordonees, 0);
...
void lectureFichierP3(char *nom_fichier, char *nom_fichier_sortie, int mode) {
    ...
    while (!feof(fichier)) {
        ...
        // AVL par date
        if (mode == 0){
            arbre = insereAVLP3(arbre, valeur1, valeur2, 0, 0, date, 0, &h);
        }
        else if (mode == 1){
            arbre = insereABRP3(arbre, valeur1, valeur2, 0, 0, date, 0);
        }
        else{
            //chainon
...
void lectureFichierT3(char *nom_fichier, char *nom_fichier_sortie, int mode) {
    ...
    while (!feof(fichier)) {
        ...
        // AVL par date
        if (mode == 0){
            arbre = insereAVLP3(arbre, valeur1, valeur2, 0, 0, date, 0, &h);; //La même chose pour T3 et P3
        }
        else if (mode == 1){
            arbre = insereABRP3(arbre, valeur1, valeur2, 0, 0, date, 0);; //La même chose pour T3 et P3
        }
        else{
            //chainon
            liste = insertionListeP3(liste, valeur1, valeur2, 0, 0, date, 0);
